...
class EDFDNet(nn.Module):
    ...
    def __init__(self, num_feat, dict_path):
        ...
        # dict
        ## FIXME: Use path and not file
        # self.dict = torch.load(dict_path)
        self.dict = dict_path
        # print("\n\nLoaded new dictionary")
        # print("Manual lookup in dictionary: ", self.dict["256"]["left_eye"].shape)
    ...
    def swap_feat(self, vgg_feat, updated_feat, dict_feat, location, part_name, f_size):
        ...
        # updated_feat[:, :, location[1]:location[3], location[0]:location[2]] = swap_feat
        # updated_feat[:, :, location[1]:location[3], location[0]:location[2]] = part_feat
        return updated_feat
    ...
    def forward(self, x, part_locations):
        ...
        for vgg_layer, f_size in zip(self.vgg_layers, self.feature_sizes):
            ...
            print(f"VGG layer {vgg_layer} shape: ", vgg_feat.shape)
            updated_feat = vgg_feat.clone()

            # swap features from dictionary
            for part_idx, part_name in enumerate(self.parts):
                # print(f"forward dict[{f'{f_size}'}][{part_name}]: ", dict_features[part_name].shape)
                # print(f"forward dict_features[{part_name}]: ", dict_features[part_name][0, 0])
