...
class FaceRestorationHelper(object):
    ...
    def detect_faces(self, img_path, upsample_num_times=1, only_keep_largest=False):
        ...
        Args:
            ...
            upsample_num_times (int): Upsamples the reference_image before running the
                face detector
    ...
    def get_face_landmarks_68(self):
        ...
        Should only have one face at most in the cropped reference_image.
        """
        num_detected_face = 0
        for idx, face in enumerate(self.cropped_faces):
            ...
            if len(det_face) == 0:
                print(f'Cannot find faces in cropped reference_image with index {idx}.')
